...
const Name = {
  length: varuint32,
  data: buffer(({ current }) => current.length.toNumber())
}
...
const export_entry = {
  length: varuint32,
  name: buffer(({ current }) => current.length.toNumber()),
  kind: external_kind,
  index: varuint32
}
...
function onFunctionCodeSize({ current }) {
  let size = current.size.toNumber()

  size -= (current.locals.count.byteLength() || 1)

  for (const entry of current.locals.types) {
...
const SectionCode = {
  count: varuint32,
  bodies: array(function_body, ({ current }) => current.count.toNumber())
}
